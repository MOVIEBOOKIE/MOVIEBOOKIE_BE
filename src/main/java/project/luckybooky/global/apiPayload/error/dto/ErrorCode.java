...
public enum ErrorCode implements BaseStatus {
  ...
  HTTP_MESSAGE_NOT_READABLE(HttpStatus.BAD_REQUEST, "G005",
      "request message body가 없거나, 값 타입이 올바르지 않습니다."),
  ...
  KAKAO_JSON_PARSE_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "KAKAO_500",
      "카카오 응답 JSON 파싱 중 오류가 발생했습니다."),
  ...
  LOCATION_ALLOWED_START_TIMES_REQUIRED(HttpStatus.BAD_REQUEST, "LOCATION_404",
      "시작 시간 제한 장소는 allowedStartTimes가 필요합니다."),
  LOCATION_ALLOWED_START_TIMES_MUST_BE_EMPTY(HttpStatus.BAD_REQUEST, "LOCATION_405",
      "시작 시간 제한이 없는 장소는 allowedStartTimes를 비워야 합니다."),
  ...
  USER_WITHDRAWAL_WITH_ACTIVE_EVENTS(HttpStatus.BAD_REQUEST, "PARTICIPATION_405",
      "진행 중인 이벤트가 있어 탈퇴할 수 없습니다. 모든 이벤트를 취소한 후 다시 시도해주세요."),
  NO_EVENT_PARTICIPANTS(HttpStatus.BAD_REQUEST, "PARTICIPATION_404", "해당 이벤트에는 아직 참여자가 없습니다."),

  // Notification 관련
  NOTIFICATION_FCM_TOKEN_NOT_FOUND(HttpStatus.BAD_REQUEST, "NOTIFICATION_400",
      "FCM 토큰이 등록되지 않았습니다."),
  NOTIFICATION_SEND_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "NOTIFICATION_500",
      "알림 전송 중 오류가 발생했습니다."),
  ...
  NOTIFICATION_SAVE_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "NOTIFICATION_503",
      "알림 내역 저장에 실패했습니다."),
  ...
  HOST_CERTIFICATION_EMAIL_NOT_FOUND(HttpStatus.NOT_FOUND, "HOST_CERTIFICATION_EMAIL_404",
      "호스트의 인증 이메일이 존재하지 않습니다"),

  // FCM 관련
  FCM_TOKEN_REGISTER_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "NOTIFICATION_503",
      "FCM 토큰 등록에 실패했습니다."),
  FCM_INITIALIZATION_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "NOTIFICATION_504",
      "FCM 초기화에 실패했습니다."),
