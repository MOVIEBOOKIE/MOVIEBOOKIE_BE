...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("SELECT e FROM Event e JOIN e.category c WHERE e.isPublic = true AND c.categoryName = :categoryName AND e.eventStatus='RECRUITING' ORDER BY e.recruitmentEnd")
    Page<Event> findByCategoryName(@Param("categoryName") String categoryName, Pageable pageable);

    @Query("SELECT e FROM Event e WHERE e.isPublic = true AND e.eventStatus='RECRUITING' ORDER BY (e.currentParticipants * 1.0) / e.maxParticipants DESC")
